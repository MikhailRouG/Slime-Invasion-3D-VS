...
static D3D11_VIEWPORT g_Viewport{}; //ビューボート設定用
...
bool Direct3D_Initialize(HWND hWnd)
{
	...
	//swap_chain_desc.SwapEffect = DXGI_SWAP_EFFECT_DISCARD;//ベンチマークテスト用
	...
	bd.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD; ////OP…オペランド(演算子) ADD…+
	...
	bd.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD; //OP…オペランド(演算子)
	//SrcA * 1 + DestA * 0
...
void Direct3D_Clear()
{
	float clear_color[4] = { 0.97f, 0.67f, 0.65f, 1.0f }; //0~1
